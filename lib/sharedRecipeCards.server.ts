...
/* ── Public read (REST, no Admin SDK in this project) ────────────────────
   ...
   Server-only, and deliberately its own module: `cache` below is exported
   from React only under the `react-server` condition, so it's `undefined` in
   a client bundle. This file is imported only by server components
   (app/print/[slug]/page.tsx), so a module-scope `cache(...)` call here is
   safe — it would throw at import time if a client component ever pulled
   this in. Creating a share link (the old admin-only write path) is gone;
   this file is the read half that keeps already-published links working. */
