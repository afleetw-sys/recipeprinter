...
export async function parseImages(images: string[]): Promise<Recipe> {
  try {
    ...
    // A photographed card is the cook's own writing, so its notes become the recipe's note.
    const notes = sourceNotesFromResponse(data);
    return notes ? { ...recipe, note: notes } : recipe;
  } catch (err) {
    throw friendlyError(
      err,
 ...
 *
 * CookPilot's parser reads that paste now (it used to be read here by a second, local parser, which
 * was deleted once the shared one handled every case it did — see CLAUDE.md, "The parser lives in
 * CookPilot"). What reaches this message is text with no heading, no numbered step and no measured
 * ingredient — so asking for a heading is both true and the thing that would change the answer.
 */
...
export async function parseText(rawText: string): Promise<Recipe> {
  ...
  try {
    ...
    // Pasted text is the cook's own writing, so its notes section becomes the recipe's note.
    const notes = sourceNotesFromResponse(data);
    return notes ? { ...recipe, note: notes } : recipe;
  } catch (err) {
