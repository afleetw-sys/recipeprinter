...
/**
 * The studio: one project, at its own address.
 *
 * This used to be `/print`, a single unnamed workspace whose contents were
 * whatever happened to be in session storage, with `?project=` bolted on as the
 * one way to name a specific document. That made two classes of URL for the
 * same screen, and left the common case — the book you are actually working on
 * — with no address at all. You could not link to it, bookmark it, or reach it
 * with the back button.
 *
 * Now every project is `/projects/<id>` and the id is a required part of the
 * route. Note what that id does NOT imply: see `projectLoadId` below, and the
 * autosave rules further down. An id is an address, not a decision to keep
 * something.
 */
export function Studio({ projectId: routeProjectId }: { projectId: string }) {
  useEffect(() => {
  ...
  // a return to the studio restores the same selection. `null` (not hydrated)
  ...
  /**
   * False to begin with, because at first render nothing is known yet.
   *
   * Under `?project=` this could be seeded from the URL: the param's presence
   * WAS the decision to load. A path id can't answer that on its own — whether
   * it needs fetching depends on the working copy, which hasn't hydrated at
   * this point. The loader below sets this the moment it commits to a fetch,
   * and until then the deck shows its own "Preparing…" state, which is the
   * honest label for "we don't know yet".
   */
  ...
  /**
   * The project this URL names, when it is NOT the working copy already open.
   *
   * Everything below branches on this the way it used to branch on `?project=`,
   * and the meaning is unchanged: non-null means "go and fetch a saved document
   * before rendering anything", null means "what is in front of you is already
   * the right project".
   *
   * The difference is that the answer is now derived rather than read. Under
   * `?project=` the URL only carried an id when it wanted a load, so the two
   * questions — which project is this, and does it need fetching — collapsed
   * into one. A path segment always carries an id, so they come apart: opening
   * the working copy at its own address must not re-fetch it from the account
   * and overwrite unsaved edits with the last-saved revision.
   *
   * Null until the meta hydrates, because `cookbookProjectId` is a freshly
   * minted uid before then and every id would look foreign. The loaders all
   * gate on `projectMeta.hydrated` too, so they simply wait rather than
   * momentarily deciding the wrong thing.
   */
  const projectLoadId =
    projectMeta.hydrated && routeProjectId && routeProjectId !== cookbookProjectId
      ? routeProjectId
      : null;
  ...
  function currentProject(): PrintProject | null {
    ...
    return assemblePrintProject({
      ...
      id: savedProjectIdRef.current ?? cookbookProjectId ?? projectLoadId,
  ...
  function handleRetrySave() {
    ...
    if (isCookbookDocument) {
      ...
      if (loadNewer && savedProjectId) {
        window.location.assign(`/projects/${encodeURIComponent(savedProjectId)}`);
        return;
      }
   ...
   */
  usePendingImport({
    // Waits for the meta, not just the queue: `projectLoadId` reads null until
    // then, so acting early would consume the stashed import into a working
    // copy that the loader below is about to overwrite — dropping the recipe
    // the person just pasted.
    enabled: projectMeta.hydrated && !projectLoadId,
  ...
  useEffect(() => {
    if (!projectLoadId || !cookPilotAuthReady || !projectMeta.hydrated || !queue.hydrated) return;
     ...
     * shelf exists.
     */
    const shelved = loadLocalProject(projectLoadId);
    ...
    setProjectAccess(null);
    loadPrintProject(cookPilotUser.uid, projectLoadId)
      .then((project) => {
        if (cancelled) return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [projectLoadId, cookPilotAuthReady, cookPilotUser?.uid, projectMeta.hydrated, queue.hydrated]);

  // A working copy keeps its project id in session/local storage, but only the
  // loader above knows that id already has a saved document behind it. Opening
  // the working copy at its own address — a reload, a return visit, the back
  // button — skips that loader by design, and therefore left `savedProjectIdRef`
  // null on a book that was already saved, so the next autosave took the
  // adoption path. Reattach to the existing document before any save can run.
  useEffect(() => {
    if (projectLoadId) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [projectLoadId, cookPilotAuthReady, cookPilotUser?.uid, projectMeta.hydrated, cookbookProjectId]);

  useEffect(() => {
    if (!projectMeta.hydrated || projectLoadId || !queue.hydrated) return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [projectLoadId, idsParam, queue.hydrated, projectMeta.hydrated]);
  ...
  if (items === null || projectLoading || cookbookAccessStatus === "loading") {
    return (
      <div className="h-full flex flex-col">
        ...
        <RecipeLoadingState
          className="flex-1"
          label={projectLoadId ? "Loading your project…" : "Preparing…"}
        />
      </div>
    );
  }
